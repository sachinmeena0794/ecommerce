        ...
        <>
            <div className="text-center rounded-lg text-white font-bold">
                <button
                    type="button"
                    onClick={openModal}
                    className="w-40 bg-black py-2 text-center rounded-lg text-white font-bold hover:bg-gray-800"
                >
                    Buy Now
                </button>
            </div>
                                                ...
                                                <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                                                    <form className="space-y-4 md:space-y-6" action="#">
                                                        ...
                                                        <div className="flex items-center space-x-2">
                                                            <label htmlFor="razorpay" className="block mb-2 text-sm font-medium text-gray-900">RazorPay</label>
                                                            <input type="radio" id="razorpay" name="paymentMethod" value="RazorPay" checked={paymentMethod === 'RazorPay'} onChange={() => setPaymentMethod('RazorPay')} />
                                                        </div>
                                                        <div className="flex items-center space-x-2">
                                                            <label htmlFor="cod" className="block mb-2 text-sm font-medium text-gray-900">Cash on Delivery</label>
                                                            <input type="radio" id="cod" name="paymentMethod" value="COD" checked={paymentMethod === 'COD'} onChange={() => setPaymentMethod('COD')} />
                                                        </div>
                                                    </form>
                                                    <button onClick={() => { buyNow(paymentMethod); closeModal() }} type="button" className="focus:outline-none w-40 text-white bg-black hover:bg-gray-800 outline-0 font-medium rounded-lg text-sm px-5 py-2.5">Order Now</button>
                                                </div>
                                            </div>
                                        </div>
                                    </section>
